  ...
  <div>
    ...
    <div class="my-info">
      ...
      <div class="fans-box">
        ...
        <span @click="$toast('哈哈就这么多')">
          <p>{{userinfo.thing_count ?userinfo.thing_count: '我很高冷'}}</p>
          <p>动态</p>
        </span>
        <span @click="$toast('哈哈就这么多')">
          <p>{{userinfo.fun_count ?userinfo.fun_count:'be my first'}}</p>
          <p>粉丝</p>
        </span>
      </div>
    </div>
      ...
      <van-tab  title="动态">
        <div class="message-box" v-for="(item,index) in things" :key="index">
          <div class="message-head">
            ...
            <span class="time delete" @click="deletedThing(item.id)">删除</span>
          </div>
...
export default {
  ...
  methods:{
    ...
    async getThingsList() {
      ...
      let {data: res} = await this.$http.get('/thing/list', {params: {page: this.thingspageNum, limit: 10}});
      if (res.code === 0) {
    ...
    async deletedThing($id) {
      let {data: res} = await this.$http.delete('/thing/'+$id);
      if (res.code === 0) {
        this.$toast('删除成功')
        this.randomThings()
      }
    ...
    async getMyArticleList() {
      let {data: res} = await this.$http.get('/article/list/'+this.userinfo.id, {params: {page: this.mypage, limit: 10}});
      if (res.code === 0) {
